...
const IMAGE_DATA_URL = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8Xw8AAn8B9UjLhXQAAAAASUVORK5CYII=';
describe('ImageBitmap', () => {
    ...
    describe('constructor()', () => {
        it('Throws error if constructor is called directly.', () => {
            expect(() => new window.ImageBitmap()).toThrowError('Illegal constructor');
        });
        it('Throws error if source is not valid.', async () => {
            await expect(() => window.createImageBitmap({})).rejects.toThrowError(new TypeError(`Failed to execute 'createImageBitmap' on 'Window': The provided value is not of type '(Blob or HTMLCanvasElement or HTMLImageElement or HTMLVideoElement or ImageBitmap or ImageData or OffscreenCanvas or SVGImageElement or VideoFrame)'.`));
        });
        it('Throws error if source is an HTMLVideoElement.', async () => {
            const video = document.createElement('video');
            await expect(() => window.createImageBitmap(video)).rejects.toThrowError(new window.DOMException("Failed to execute 'createImageBitmap' on 'Window': HTMLVideoElement is not supported as an image source yet in Happy DOM.", DOMExceptionNameEnum.invalidStateError));
        });
        ...
        it('Supports ImageBitmap as source.', async () => {
            const drawnImages = [];
            const mockContext = {
                drawImage: (image, dx, dy, dw, dh) => drawnImages.push({ image, dx, dy, dw, dh })
            };
            const mockAdapter = {
    ...
    describe('close()', () => {
        it('Clears internal data.', async () => {
            ...
            image.src = IMAGE_DATA_URL;
            const imageBitmap = await window.createImageBitmap(image, 800, 600);
            imageBitmap[PropertySymbol.canvas] = {};
            imageBitmap.close();
...
//# sourceMappingURL=ImageBitmap.test.js.map