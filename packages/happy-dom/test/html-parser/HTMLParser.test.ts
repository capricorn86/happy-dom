...
describe('HTMLParser', () => {
	...
	describe('parse()', () => {
		it('Parses HTML with a single <div>.', () => {
			const root = new HTMLParser(window).parse('<div></div>');
		...
		it('Parses HTML with a single <div> with attributes.', () => {
			const root = new HTMLParser(window).parse(
...
`;
		...
		<body>
			...
			const root = new HTMLParser(window, { mode: HTMLParserModeEnum.htmlDocument }).parse(html);
...
<html>
    ...
    <body>
		...
		it('Does not parse the content of script and style elements.', () => {
			...
			const root2 = new HTMLParser(window, { mode: HTMLParserModeEnum.htmlDocument }).parse(
				`<html>
                    <head>
                        <title>Title</title>
                    </head>
		...
		it('Handles unclosed regular elements.', () => {
			const root = new HTMLParser(window).parse(`<div>test`);
		...
		it('Parses conditional comments', () => {
			...
			for (const html of testHTML) {
				const root = new HTMLParser(window, { mode: HTMLParserModeEnum.htmlFragment }).parse(html);
		...
		it('Parses comments with dash in them.', () => {
			const root = new HTMLParser(window).parse('<!-- comment with - in - it -->');
			expect(root.childNodes.length).toBe(1);
		...
		it('Can read text with ">" in it.', () => {
			const root = new HTMLParser(window).parse(`<span>1 > 0</span>`);
		...
		it('Parses attributes with URL without apostrophs.', () => {
			const root = new HTMLParser(window).parse(`<a href=http://www.github.com/path>Click me</a>`);
		...
		it('Parses attributes with single apostrophs.', () => {
			const root = new HTMLParser(window).parse(`<div key1='value1' key2='value2'>Test</div>`);
		...
		it('Handles multiple <!DOCTYPE>.', () => {
			const root = new HTMLParser(window, { mode: HTMLParserModeEnum.htmlDocument }).parse(
		...
		it('Handles multiple <body> tag names', () => {
			const root1 = new HTMLParser(window, { mode: HTMLParserModeEnum.htmlDocument }).parse(
				`<body>
                    <div>Test 1</div>
                </body>
		...
		it('Ignores processing instructions as comments when parser mode is XML.', () => {
			const root = new HTMLParser(window, { mode: HTMLParserModeEnum.xmlDocument }).parse(
				`<?xml version="1.0" encoding="UTF-8"?>
                <div>
                    <div>Test</div>
                </div>`
		...
		it('Handles namespaced XML', () => {
			...
			expect(root.children[0].namespaceURI).toBe('http://www.your.example.com/xml/person');
