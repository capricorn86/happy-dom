...
describe('TextTrack', () => {
	...
	describe('constructor()', () => {
		it('Should throw an error if the "illegalConstructor" symbol is not sent to the constructor', () => {
			expect(() => new TextTrack()).toThrow(new TypeError('Illegal constructor'));
		});

		it('Should not throw an error if the "illegalConstructor" symbol is provided', () => {
			expect(() => new TextTrack(PropertySymbol.illegalConstructor)).not.toThrow();
		});

		it('Is an instance of EventTarget', () => {
			const textTrackList = new TextTrack(PropertySymbol.illegalConstructor);
