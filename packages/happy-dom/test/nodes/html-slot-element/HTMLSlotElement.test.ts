...
describe('HTMLSlotElement', () => {
	...
	describe('Element.assignedSlot', () => {
		it('Returns the named slot the element is assigned to.', () => {
			...
			customElementWithNamedSlots.appendChild(element);

			expect(element.assignedSlot === slot).toBe(true);
			expect(slot.assignedNodes().includes(element)).toBe(true);
		});
		...
		it('Returns null when the element has a slot name that no slot matches.', () => {
			const element = document.createElement('div');
		...
		it('Returns null for an element inside the shadow root.', () => {
			...
			expect(slot.assignedSlot).toBe(null);
		});
		...
		it('Returns the manually assigned slot when slot assignment is "manual".', () => {
			...
			const slot = <HTMLSlotElement>shadowRoot.querySelector('slot');
	...
	describe('Text.assignedSlot', () => {
		it('Returns the default slot the text node is assigned to.', () => {
			...
			expect(text.assignedSlot === slot).toBe(true);
			expect(slot.assignedNodes().includes(text)).toBe(true);
		});
